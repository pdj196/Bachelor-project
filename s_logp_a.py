...
for n in range(len(target)):
  ...
  for run in range (runs):
    ...
    population = spa.kekulize_list(np.random.choice(ipr.remove_smiles(smiles_list,-4,2), population_size))
    logp_scores = spa.calculate_score(population)
    scores = spa.gaussian(population, target[n], sigma) 
    fitness = spa.calculate_normalized_fitness(scores)

    for generation in range(generations):
      mating_pool = spa.make_mating_pool(population, fitness, mating_pool_size)  
      new_population = spa.make_new_population(population_size, mating_pool, max_length_child, mutation_rate, symbols)

      count = new_population[len(new_population) - 1]
      count_list.append(count)

      new_population = new_population [0:len(new_population) - 2]
      new_logp_scores = spa.calculate_score(new_population)
      new_scores = spa.gaussian(new_population, target[n], sigma)
